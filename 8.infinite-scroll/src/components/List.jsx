...
export default function List() {
  ...
  return (
    ...
    className='block mb-4' htmlFor="search">Look for images...
      <input
      ref={searchRef}
      ...
      {!photosApiData.loader && photosApiData.photos.length !== 0 && photosApiData.photos.map((photo, index) => {
        if(photosApiData.photos.length === index + 1){
          return(
            ...
            key={photo.id}>
              <img 
              className="w-full h-full object-cover border-4 border-red-400"
    ...
    {photosApiData.maxPages === pageNumber && <p className="my-10 text-center">No more images to show for that query</p>}
    </>
  )
}

...
export default function SideNotes() {
  ...
  return (
    <aside className="shrink-0 bg-slate-100 text-slate-900 w-[275px] flex flex-col items-center pt-2 border-r border-gray-300">
      <p className="w-full text-center py-6 px-4 text-2xl text-slate-800 font-semibold border-b border-gray-400">Mes notes</p>
      <ul className="w-full divide-y divide-slate-300">
        {notes.list && notes.list.map(note =>(
          <li key={note.id} className="relative hover:bg-slate-200 cursor-pointer">
            <Link
            to={`/note/${note.id}`}
